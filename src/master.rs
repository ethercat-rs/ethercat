...
macro_rules! ioctl {
    ($m:expr, $nr:expr) => { ioctl!($m, $nr, 0 as *mut i8) };
    ($m:expr, $nr:expr, @$arg:expr) => { ioctl!($m, $nr, $arg as *mut i8) };
    ($m:expr, $nr:expr, $arg:expr) => {{
        let res = unsafe { libc::ioctl($m.file.as_raw_fd(), $nr, $arg as *mut _) };
...
impl Master {
    pub fn reserve(index: MasterIndex) -> Result<Self> {
        ...
        ioctl!(master, ec::EC_IOCTL_MODULE, &mut module_info)?;
    ...
    pub fn create_domain(&mut self) -> Result<DomainHandle> {
        let index = ioctl!(self, ec::EC_IOCTL_CREATE_DOMAIN).map(|v| v as DomainIndex)?;
    ...
    pub fn domain_data(&mut self, index: DomainHandle) -> &mut [u8] {
        ...
        if size == 0 {
            size = ioctl!(self, ec::EC_IOCTL_DOMAIN_SIZE, @ix).unwrap() as usize;
            offset = ioctl!(self, ec::EC_IOCTL_DOMAIN_OFFSET, @ix).unwrap() as usize;
    ...
    pub fn activate(&mut self) -> Result<()> {
        ...
        ioctl!(self, ec::EC_IOCTL_ACTIVATE, &mut data)?;
    ...
    pub fn deactivate(&mut self) -> Result<()> {
        ioctl!(self, ec::EC_IOCTL_DEACTIVATE)?;
        self.domains.clear();
        self.map = None;
        Ok(())
    }
    ...
    pub fn set_send_interval(&mut self, mut interval_us: usize) -> Result<()> {
        ioctl!(self, ec::EC_IOCTL_SET_SEND_INTERVAL, &mut interval_us).map(|_| ())
    }

    pub fn send(&mut self) -> Result<usize> {
        let mut sent = 0;
        ioctl!(self, ec::EC_IOCTL_SEND, &mut sent)?;
        Ok(sent)
    }

    pub fn receive(&mut self) -> Result<()> {
        ioctl!(self, ec::EC_IOCTL_RECEIVE).map(|_| ())
    }

    pub fn reset(&mut self) -> Result<()> {
        ioctl!(self, ec::EC_IOCTL_RESET).map(|_| ())
    }

    pub fn state(&self) -> Result<MasterState> {
        let mut data = ec::ec_master_state_t::default();
        ioctl!(self, ec::EC_IOCTL_MASTER_STATE, &mut data)?;
        Ok(MasterState {
    ...
    pub fn get_info(&self) -> Result<MasterInfo> {
        ...
        ioctl!(self, ec::EC_IOCTL_MASTER, &mut data)?;
        Ok(MasterInfo {
    ...
    pub fn get_slave_info(&self, position: u16) -> Result<SlaveInfo> {
        ...
        ioctl!(self, ec::EC_IOCTL_SLAVE, &mut data)?;
    ...
    pub fn configure_slave(&mut self, addr: SlaveAddr, expected: SlaveId) -> Result<SlaveConfig> {
        ...
        ioctl!(self, ec::EC_IOCTL_CREATE_SLAVE_CONFIG, &mut data)?;
...
impl<'m> SlaveConfig<'m> {
    pub fn get_state(&self) -> Result<SlaveConfigState> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_STATE, &mut data)?;
        Ok(SlaveConfigState {
    ...
    pub fn config_watchdog(&mut self, divider: u16, intervals: u16) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_WATCHDOG, &mut data).map(|_| ())
    }
    ...
    pub fn config_overlapping_pdos(&mut self, allow: bool) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_OVERLAPPING_IO, &mut data).map(|_| ())
    }
    ...
    pub fn config_sync_manager(&mut self, info: &SyncInfo) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_SYNC, &mut data).map(|_| ())
    }
    ...
    pub fn clear_pdo_assignments(&mut self, sync_index: SmIndex) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_CLEAR_PDOS, &mut data).map(|_| ())
    }
    ...
    pub fn add_pdo_assignment(&mut self, sync_index: SmIndex, pdo: &PdoInfo) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_ADD_PDO, &mut data).map(|_| ())
    }
    ...
    pub fn clear_pdo_mapping(&mut self, pdo_index: PdoIndex) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_CLEAR_ENTRIES, &mut data).map(|_| ())
    }
    ...
    pub fn add_pdo_mapping(&mut self, pdo_index: PdoIndex, entry: &PdoEntryInfo) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_ADD_ENTRY, &mut data).map(|_| ())
    }
    ...
    pub fn register_pdo_entry(&mut self, index: PdoEntryIndex, domain: DomainHandle) -> Result<Position> {
        ...
        let byte = ioctl!(self.master, ec::EC_IOCTL_SC_REG_PDO_ENTRY, &mut data)?;
    ...
    pub fn register_pdo_entry_by_position(&mut self, sync_index: SmIndex, pdo_pos: u32, entry_pos: u32,
        ...
        let byte = ioctl!(self.master, ec::EC_IOCTL_SC_REG_PDO_POS, &mut data)?;
        Ok(Position { byte: byte as usize, bit: data.bit_position })
    }
    ...
    pub fn config_dc(&mut self, assign_activate: u16, sync0_cycle_time: u32, sync0_shift_time: i32,
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_DC, &mut data).map(|_| ())
    }
    ...
    pub fn add_sdo<T: SdoData>(&mut self, index: SdoIndex, data: T) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_SDO, &mut data).map(|_| ())
    }
    ...
    pub fn config_idn(&mut self, drive_no: u8, idn: u16, al_state: AlState, data: &[u8]) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_IDN, &mut data).map(|_| ())
    }
    ...
    pub fn set_emerg_size(&mut self, elements: usize) -> Result<()> {
        ...
        data.config_index = self.index;
        data.size = elements;
        ioctl!(self.master, ec::EC_IOCTL_SC_EMERG_SIZE, &mut data).map(|_| ())
    }
    ...
    pub fn pop_emerg(&mut self, target: &mut [u8]) -> Result<()> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_SC_EMERG_POP, &mut data).map(|_| ())
    }

    pub fn clear_emerg(&mut self) -> Result<()> {
        let mut data = ec::ec_ioctl_sc_emerg_t::default();
        data.config_index = self.index;
        ioctl!(self.master, ec::EC_IOCTL_SC_EMERG_CLEAR, &mut data).map(|_| ())
    }

    pub fn emerg_overruns(&mut self) -> Result<i32> {
        let mut data = ec::ec_ioctl_sc_emerg_t::default();
        data.config_index = self.index;
        ioctl!(self.master, ec::EC_IOCTL_SC_EMERG_OVERRUNS, &mut data)?;
        Ok(data.overruns)
    }
...
impl<'m> Domain<'m> {
    ...
    pub fn state(&self) -> Result<DomainState> {
        ...
        ioctl!(self.master, ec::EC_IOCTL_DOMAIN_STATE, &mut data)?;
        Ok(DomainState {
